<template>
  <div class="min-h-screen bg-gray-100 mx-[10%] my-[3%] p-3">
    <!-- Header -->
    ...
    <!-- Search Form -->
    <div v-if="showSearchForm" class="bg-white mt-[1%] p-6 rounded-lg shadow-md">
      <form class="grid grid-cols-2 gap-4">
        <div>
          <label class="block text-base font-medium text-gray-700">From</label>
          <input v-model="tempSearch.from" type="text" class="form-input text-base" />
        </div>
        <div>
          <label class="block text-base font-medium text-gray-700">To</label>
          <input v-model="tempSearch.to" type="text" class="form-input text-base" />
        </div>
        <div>
          <label class="block text-base font-medium text-gray-700">Departure Date</label>
          <input v-model="tempSearch.date" type="date" class="form-input text-base" />
        </div>
        <div>
          <label class="block text-base font-medium text-gray-700">No. of Seats</label>
          <input v-model="tempSearch.seats" type="number" class="form-input text-base" />
        </div>
      </form>
      <div class="mt-4 flex justify-end">
        <button @click="applySearch" class="bg-orange-500 text-white px-6 py-2 rounded-lg text-base">
          Apply
        </button>
      </div>
    </div>
    ...
    <div class="flex mt-6">
      ...
      <div class="w-1/4 bg-white p-4 rounded-lg shadow-md">
        ...
        <button class="text-blue-500 text-base mb-4" @click="resetFilters">
          Reset filter
        </button>

        <div v-for="(options, key) in filters" :key="key" class="mb-4">
          <button @click="toggleDropdown(key)"
            class="flex justify-between items-center w-full text-base font-medium text-gray-700 bg-gray-100 px-3 py-2 rounded-md">
            {{ key }}
          ...
          <div class="overflow-hidden transition-all duration-300 ease-in-out"
            ...
            <div class="mt-2 p-2 border rounded-lg bg-gray-50">
              <div v-for="option in options" :key="option" class="mt-2 flex items-center">
                ...
                <label :for="option" class="text-base text-gray-600">{{ option }}</label>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="w-3/4 ml-4">
        <div v-for="ticket in filteredTickets" :key="ticket.id" class="bg-white mb-6 p-6 rounded-lg shadow-md">
          <div class="flex justify-between items-start">
            <!-- Left Section -->
            <div class="w-2/3">
              <h3 class="text-xl font-semibold text-gray-800">{{ ticket.company }}</h3>
              <p class="text-base text-gray-600 mb-2">
                {{ ticket.departure }} → {{ ticket.arrival }}
              </p>
              <p class="text-base text-gray-500 mb-2">Duration: {{ ticket.duration }}</p>
              <p class="text-base text-gray-500">Seats: {{ ticket.seats }}</p>
            </div>

            <!-- Right Section -->
            <div class="w-1/3 text-right">
              <p class="text-xl font-bold text-orange-500">{{ ticket.price }} VND</p>
              <p class="text-base text-gray-500">/pax</p>
              <button class="mt-4 bg-blue-500 text-white px-6 py-2 rounded-lg text-base hover:bg-blue-600 transition">
                Book Now
              </button>
            </div>
          </div>
          ...
          <el-tabs type="card">
            <el-tab-pane label="Đặc điểm" name="features">
              <div class="flex items-center gap-4">
                <div class="flex-1">
                  <h4 class="text-base font-semibold text-gray-700 mb-2">Fleet Specification</h4>
                ...
                <div class="flex-1">
                  <h4 class="text-base font-semibold text-gray-700 mb-2">Refund & Policy</h4>
                  <ul class="text-base text-gray-600">
